...
{
	WORD a, b, c, d, e, f, g, h, i, j, t1, t2, m[NB_TURN];

	for (i = 0, j = 0; i < 16; ++i, j += 8)
		m[i] = (
			 (WORD)ctx->data[j]     << 56) |
			((WORD)ctx->data[j + 1] << 48) |
			((WORD)ctx->data[j + 2] << 40) |
			((WORD)ctx->data[j + 3] << 32) |
			((WORD)ctx->data[j + 4] << 24) |
			((WORD)ctx->data[j + 5] << 16) |
			((WORD)ctx->data[j + 6] << 8)  |
			((WORD)ctx->data[j + 7]
		);
